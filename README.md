...
- ArgoCD manages continuous delivery by syncing with the argo branch to the first cluster
- another cluster FluxCD source operator syncing flux branch
- Hashi Corp Vault is deployed to seal Nexus credentials secrets using (vault secrets operator)
- Keycloak Provides GUI (Single Sign-On) using OpenID Connect (OIDC).
